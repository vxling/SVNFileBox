...
public class SyncService : IDisposable
{
    ...
    private async Task CommitFileAsync(string filePath)
    {
        ...
        {
            _recordService.AddRecord(_currentRepo.Name, fileName, operation, "Failed", "Commit returned non-zero");
        }
    }
    ...
    private async Task PollCoreAsync()
    {
        ...
        try
        {
            ...
            var serverRev = await _svnService.GetHeadRevisionAsync(_currentRepo.Url, _currentRepo.Username, null);
            ...
            foreach (var filePath in conflictedFiles)
            {
                try
                {
                    ...
                    if (infoResult.exitCode == 0)
                    {
                        ...
                        if (dateMatch.Success && DateTime.TryParse(dateMatch.Groups[1].Value.Trim(), out var parsed))

...
public partial class MainViewModel : ObservableObject, IDisposable
{
    ...
    {
        SyncRecords.Clear();
        foreach (var r in RecordService.Records)
    ...
    public async Task InitializeAsync()
    {
        ...
        try
        {
            ...
            foreach (var repo in _configService.Config.Repositories)
                Application.Current?.Dispatcher.Invoke(() => Repositories.Add(repo));

            if (!string.IsNullOrEmpty(_configService.Config.ActiveRepositoryPath))
            {
                var active = Repositories.FirstOrDefault(r => r.Path == _configService.Config.ActiveRepositoryPath);
                if (active != null)
    ...
    partial void OnSelectedRepositoryChanged(Repository? value)
    {
        ...
        if (value != null && Directory.Exists(value.Path))
        {
            ...
            _configService.Config.ActiveRepositoryPath = value.Path;
    ...
    public async Task LoadDirectoryAsync(string path)
    {
        ...
        try
        {
            ...
            {
                // Replace with a NEW collection so PropertyChanged("Files") fires and WPF detects the change
